class ChannelQuepasa extends App.ControllerSubContent
  requiredPermission: 'admin.channel_quepasa'
  events:
    'click .js-new':     'new'
    'click .js-edit':    'edit'
  ...
  disable: (e) =>
    ...
    @ajax(
      id:   'quepasa_disable'
      type: 'POST'
      url:  "#{@apiPath}/channels_quepasa_disable"
      data: JSON.stringify(id: id)
      processData: true
      success: =>
        @load()
  ...
  enable: (e) =>
    ...
    @ajax(
      id:   'quepasa_enable'
      type: 'POST'
      url:  "#{@apiPath}/channels_quepasa_enable"
      data: JSON.stringify(id: id)
      processData: true
      success: =>
        @load()
...
class BotAdd extends App.ControllerModal
  ...
  content: ->
    content = $(App.view('quepasa/bot_add')())
...
class BotEdit extends App.ControllerModal
  head: 'Quepasa Account'
  shown: true
  buttonCancel: true

  content: ->
    content = $(App.view('quepasa/bot_edit')(channel: @channel))
  ...
  onSubmit: (e) =>
    ...
    @ajax(
      id:   'channel_quepasa_update'
      type: 'PUT'
      url:  "#{@apiPath}/channels_quepasa/#{@channel.id}"
      data: JSON.stringify(@formParams())
      processData: true
      success: =>
        @isChanged = true
...
App.Config.set('Quepasa', { prio: 5100, name: 'Quepasa', parent: '#channels', target: '#channels/quepasa', controller: ChannelQuepasa, permission: ['admin.channel_quepasa'] }, 'NavBarAdmin')
