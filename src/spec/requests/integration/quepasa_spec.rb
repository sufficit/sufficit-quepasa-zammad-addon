  ...
  describe 'request handling' do
    ...
    describe 'check_token' do
      it 'invalid token' do
        ...
        expect do
          Quepasa.check_token('invalid_token')
    ...
    describe 'create_or_update_channel' do
      ...
      it 'via http' do
        ...
        expect do
          Quepasa.create_or_update_channel(token, { group_id: group_id, welcome: 'hi!', goodbye: 'goodbye' })
        end.to raise_error(Exceptions::UnprocessableEntity)
      end
      ...
      it 'with https, valid token, host and port' do
        ...
        channel = Quepasa.create_or_update_channel(token, { group_id: group_id, welcome: 'hi!', goodbye: 'goodbye' })
    ...
    describe 'communication' do
      before do
        UserInfo.current_user_id = 1
        Channel.where(area: 'Quepasa::Bot').destroy_all
        Setting.set('http_type', 'https')
        Setting.set('fqdn', 'example.com')
      ...
      let!(:channel) { Quepasa.create_or_update_channel(token, { group_id: group_id, welcome: 'hi!', goodbye: 'goodbye' }) }
      let!(:channel2) { Quepasa.create_or_update_channel(token2, { group_id: group_id2, welcome: 'hi!', goodbye: 'goodbye' }) }
      let!(:callback_url)  { "/api/v1/channels_quepasa_webhook/#{channel.options[:callback_token]}?bid=#{channel.options[:bot][:id]}" }
      let!(:callback_url2) { "/api/v1/channels_quepasa_webhook/#{channel2.options[:callback_token]}?bid=#{channel2.options[:bot][:id]}" }
      ...
      describe 'private' do
        ...
        it 'no found message' do
          post '/api/v1/channels_quepasa_webhook', params: read_message('private', 'start'), as: :json
          expect(response).to have_http_status(:not_found)
        end

        it 'bot id is missing' do
          post '/api/v1/channels_quepasa_webhook/not_existing', params: read_message('private', 'start'), as: :json
          expect(response).to have_http_status(:unprocessable_entity)
          expect(json_response['error']).to eq('bot id is missing')
        end

        it 'invalid callback token' do
          callback_url = "/api/v1/channels_quepasa_webhook/not_existing?bid=#{channel.options[:bot][:id]}"
    ...
    def read_message(type, file)
      JSON.parse(File.read(Rails.root.join('test', 'data', 'quepasa', type, "#{file}.json")))
    end
